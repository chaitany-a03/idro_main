        ...
        const fetchMissionData = async () => {
            try {
                ...
                const enriched = await Promise.all(baseAllocations.map(async (alloc, idx) => {
                    ...
                    let distKm = Math.round(haversineKm(originLatLng, destinationLatLng));
                    let etaHours = (distKm / 45).toFixed(1);

                    const pseudoRandom = (min, max, suffix = "dist") => min + (Math.abs([...(seed + suffix)].reduce((a, c) => (a * 31 + c.charCodeAt(0)) | 0, 0)) % (max - min + 1));

                    if (u.includes("IMMEDIATE") || u.includes("NOW")) {
                        distKm = 0;
                        etaHours = "0";
                    } else if (/\b6\b|SIX|QUICK/.test(u)) {
                        distKm = pseudoRandom(50, 60, "dist");
                        etaHours = (pseudoRandom(40, 60, "eta") / 10).toFixed(1);
                    } else if (/\b12\b|TWELVE|STAGED/.test(u)) {
                        distKm = pseudoRandom(80, 100, "dist");
                        etaHours = (pseudoRandom(90, 110, "eta") / 10).toFixed(1);
                    } else {
                        distKm = pseudoRandom(200, 350, "dist");
                        etaHours = (pseudoRandom(200, 230, "eta") / 10).toFixed(1);
                    }
                    return {
                        ...alloc,
                        status: statusLabel,
